...
import games.rednblack.editor.renderer.ecs.annotations.DelayedComponentRemoval;
import games.rednblack.editor.renderer.ecs.utils.Bag;

import static com.badlogic.gdx.utils.reflect.ClassReflection.isAnnotationPresent;
...
public class ComponentMapper<A extends Component> extends BaseComponentMapper<A> {
    ...
    /**
     * Holds all components of given type in the engine.
     */
    ...
    public ComponentMapper(Class<A> type, Engine engine) {
        super(engine.getComponentManager().typeFactory.getTypeFor(type));
        ...
        if (engine.isAlwaysDelayComponentRemoval() || isAnnotationPresent(type, DelayedComponentRemoval.class))
            purgatory = new DelayedComponentRemover<>(components, pool, engine.batchProcessor);
        else
            purgatory = new ImmediateComponentRemover<>(components, pool);

        createTransmuter = new EntityTransmuterFactory(engine).add(type).build();
        removeTransmuter = new EntityTransmuterFactory(engine).remove(type).build();
    }
     ...
     * <p>
     * Components annotated with {@link DelayedComponentRemoval} (or all components if {@link Engine#isAlwaysDelayComponentRemoval}
