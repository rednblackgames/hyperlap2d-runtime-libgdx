...
import games.rednblack.editor.renderer.ecs.utils.ConverterUtil;
...
class ReferenceTracker {
	...
	ReferenceTracker(Engine engine) {
		this.engine = engine;
	}

	void inspectTypes(Engine engine) {
		clear();
	...
	private boolean findReferences(int entityId, Field f, BitVector referencedIds) {
		Component c = engine.getEntity(entityId).getComponent(f.getDeclaringClass());
		if (c == null)
			return false;
