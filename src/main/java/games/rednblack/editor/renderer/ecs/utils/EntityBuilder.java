...
import games.rednblack.editor.renderer.ecs.managers.PlayerManager;
import games.rednblack.editor.renderer.ecs.managers.TagManager;
import games.rednblack.editor.renderer.ecs.managers.UuidEntityManager;
...
public class EntityBuilder {
	...
	public EntityBuilder(Engine engine) {
		this.engine = engine;
		edit = engine.createEntity().edit();
	}

	/** Begin building new entity based on archetype.*/
	public EntityBuilder(Engine engine, Archetype archetype) {
		this.engine = engine;
		edit = engine.createEntity(archetype).edit();
	}
	...
	protected <T extends BaseSystem> T resolveManager(Class<T> type) {
		final T teamManager = engine.getSystem(type);
		if ( teamManager == null ) {
			throw new RuntimeException("Register " + ClassReflection.getSimpleName(type) + " with your artemis engine.");
		}
		return teamManager;
	}
}
