...
import games.rednblack.editor.renderer.ecs.EntityTransmuter.TransmuteOperation;
 ...
 * Calling {@link Entity#edit() edit()} on the Entity returned by {@link Engine#createEntity(Archetype)}
 * nullifies this optimization.  
 * <p>
 * Generated archetypes provide a blueprint for quick entity creation.
 * Instance generated entities using {@link Engine#createEntity(Archetype)}
 * 
 * @since 0.7
 */
public class ArchetypeBuilder {
	...
	/**
	 * Create a new engine specific instance of Archetype based on the current state.
	 *
	 * @param engine applicable domain of the Archetype.
	 * @return new Archetype based on current state
	 */
	public Archetype build(Engine engine) {
		return build(engine, null);
	}
	 ...
	 * @return new Archetype based on current state
	 */
	public Archetype build(Engine engine, String name) {
		ComponentType[] types = resolveTypes(engine);
	...
	private ComponentType[] resolveTypes(Engine engine) {
