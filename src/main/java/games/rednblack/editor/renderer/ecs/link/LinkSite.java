...
import static games.rednblack.editor.renderer.ecs.annotations.LinkPolicy.Policy.CHECK_SOURCE_AND_TARGETS;
...
abstract class LinkSite implements EntitySubscription.SubscriptionListener {
	...
	protected LinkSite(Engine engine,
					   ComponentType type,
					   Field field,
		...
		mapper = engine.getMapper(type.getType());

		activeEntityIds = engine.getAspectSubscriptionManager().get(all()).getActiveEntityIds();

		AspectSubscriptionManager subscriptions = engine.getAspectSubscriptionManager();
