...
import games.rednblack.editor.renderer.ecs.utils.ShortBag;
...
public final class EntityTransmuter {
	...
	public EntityTransmuter(Engine engine, Aspect.Builder aspect) {
		this(engine, engine.getAspectSubscriptionManager().get(aspect).getAspect());
	}

	EntityTransmuter(Engine engine, Aspect aspect) {
		this(engine, new BitVector(aspect.allSet), new BitVector(aspect.exclusionSet));
	}

	EntityTransmuter(Engine engine, BitVector additions, BitVector removals) {
		em = engine.getEntityManager();
		entityToIdentity = engine.getComponentManager().entityToIdentity;
		batchProcessor = engine.batchProcessor;
		operations = new Bag<TransmuteOperation>(TransmuteOperation.class);

		factory = new Factory(engine, additions, removals);
	}
	...
	static class Factory {
		...
		Factory(Engine engine, BitVector additions, BitVector removals) {
			this.cm = engine.getComponentManager();
