...
import games.rednblack.editor.renderer.ecs.annotations.UnstableApi;
import games.rednblack.editor.renderer.ecs.injection.CachedInjector;
import games.rednblack.editor.renderer.ecs.injection.FieldHandler;
import games.rednblack.editor.renderer.ecs.injection.FieldResolver;
import games.rednblack.editor.renderer.ecs.injection.InjectionCache;
import games.rednblack.editor.renderer.ecs.utils.Bag;
import games.rednblack.editor.renderer.ecs.utils.IntBag;
import games.rednblack.editor.renderer.ecs.utils.Sort;
...
public class EngineConfigurationBuilder {
    ...
    /**
     * Assemble engine with systems.
     *
     * Deprecated: Engine Configuration
     */
    ...
    private void registerInvocationStrategies(EngineConfiguration config) {
     ...
     */
    protected void registerFieldResolvers(EngineConfiguration config) {
     ...
     */
    private void registerSystems(EngineConfiguration config) {
     ...
     */
    public EngineConfigurationBuilder alwaysDelayComponentRemoval(boolean value) {
     ...
     * @return this
     */
    public EngineConfigurationBuilder register(FieldResolver... fieldResolvers) {
     ...
     * @return this
     */
    public EngineConfigurationBuilder register(SystemInvocationStrategy strategy) {
     ...
     * @return this
     */
    public final EngineConfigurationBuilder dependsOn(Class... types) {
     ...
     * @return this
     * @throws EngineConfigurationException if unsupported classes are passed or plugins are given a priority.
     */
    @SuppressWarnings("unchecked")
    public final EngineConfigurationBuilder dependsOn(int priority, Class... types) {
                    ...
                    default:
                        if (ClassReflection.isAssignableFrom(ArtemisPlugin.class, type)) {
                            if (priority != Priority.NORMAL) {
                                throw new EngineConfigurationException("Priority not supported on plugins.");
                            }
                            dependsOnPlugin(type);
                        } else {
                            throw new EngineConfigurationException("Unsupported type. Only supports systems.");
                        }
                }
            } catch (ReflectionException e) {
                throw new EngineConfigurationException("Unable to instance " + type + " via reflection.", e);
            }
        }
        return this;
    }
    ...
    private void dependsOnPlugin(Class<? extends ArtemisPlugin> type) throws ReflectionException {
        ...
        if (ClassReflection.isAbstract(type)) {
            if (!anyAssignableTo(plugins, type)) {
                throw new EngineConfigurationException("Implementation of " + type + " expected but not found. Did you forget to include a plugin? (for example: logging-libgdx for logging-api)");
            }
        } else {
     ...
     * @return this
     * @throws EngineConfigurationException if registering the same class twice.
     */
    public EngineConfigurationBuilder with(int priority, BaseSystem... systems) {
     ...
     * @throws EngineConfigurationException if registering the same class twice.
     */
    public EngineConfigurationBuilder with(BaseSystem... systems) {
     ...
     * @return this
     * @throws EngineConfigurationException if type is added more than once.
     */
    public EngineConfigurationBuilder with(ArtemisPlugin... plugins) {
        addPlugins(plugins);
        return this;
    }
    ...
    private void addSystems(int priority, BaseSystem[] systems) {
        for (BaseSystem system : systems) {
            ...
            if (containsType(this.systems, system.getClass())) {
                throw new EngineConfigurationException("System of type " + system.getClass() + " registered twice. Only once allowed.");
            }
    ...
    private void addPlugins(ArtemisPlugin[] plugins) {
        for (ArtemisPlugin plugin : plugins) {
            ...
            if (containsType(this.plugins, plugin.getClass())) {
                throw new EngineConfigurationException("Plugin of type " + plugin.getClass() + " registered twice. Only once allowed.");
            }
