 ...
 * <p>
 * Only one component manager exists per {@link Engine} instance,
...
public class ComponentManager extends BaseSystem {
	...
	void registerComponentType(ComponentType ct, int capacity) {
		...
		ComponentMapper mapper = new ComponentMapper(ct.getType(), engine);
   ...
   /**
	...
	/**
	 * Synchronizes new subscriptions with {@link Engine} state.
	 *
	...
	void synchronize(EntitySubscription es) {
		...
		for (Entity e : engine.getEntityManager().entities) {
