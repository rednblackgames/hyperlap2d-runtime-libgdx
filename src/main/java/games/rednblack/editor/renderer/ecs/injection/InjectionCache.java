...
public class InjectionCache {
	...
	public CachedClass getCachedClass(Class<?> clazz) throws ReflectionException {
		...
		if (cachedClass == null) {
			...
			if (cachedClass.wireType == WireType.IGNORED && clazz != Object.class) {
				...
				setWireAnnotation(cachedClass, Objects.requireNonNull(ReflectionUtil.getAnnotation(clazz, Wire.class)));
			}
	...
	public CachedField getCachedField(Field field) {
		...
		if (cachedField == null) {
			if (field.isAnnotationPresent(Wire.class)) {
				final Wire wire = ReflectionUtil.getAnnotation(field, Wire.class);
	...
	public ClassType getFieldClassType(Class<?> fieldType) {
		...
		if (injectionType == null) {
			...
			} else if (ClassReflection.isAssignableFrom(Engine.class, fieldType)) {
				injectionType = ClassType.ENGINE;
			} else {
