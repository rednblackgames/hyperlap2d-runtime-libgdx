...
import games.rednblack.editor.renderer.ecs.utils.IntDeque;
...
public class EntityManager extends BaseSystem {
	...
	void clean(IntBag pendingDeletion) {
		...
		for(int i = 0, s = pendingDeletion.size(); s > i; i++) {
			...
			// it is added to the engine, ie; created and deleted
			// before Engine#process has been called
	 ...
	 * return an entity even if it isn't active in the engine, </em> use
	 ...
	 * the engine for this method to work. This method does nothing if it fails.</p>
	 *
	 * <p>For the reset to take effect, a new {@link Engine#process()} must initiate.</p>
	 *
	 * @return true if entity id was successfully reset.
	 *
	 */
	public boolean reset() {
		int count = engine.getAspectSubscriptionManager()
			.get(all())
			.getActiveEntityIds()
			.cardinality();
	...
	/**
	 * Instantiates an Entity without registering it into the engine.
	 * @param id The ID to be set on the Entity
	 */
	private Entity createEntity(int id) {
		Entity e = new Entity(engine, id);
